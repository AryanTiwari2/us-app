...
<div className="flex-1 overflow-y-auto px-4 lg:px-6 pb-4 lg:pb-6 max-h-[calc(100vh-28rem)] lg:max-h-[calc(100vh-24rem)] flex flex-col gap-2 lg:gap-3 scrollbar-thin scrollbar-thumb-[color.bg]/50 scrollbar-track-white/50 [-webkit-scrollbar-thumb:rounded] [-webkit-scrollbar-track:rounded] snap-y snap-mandatory">
  {filtered.length === 0 ? (
    ...
    filtered.map(movie => (
      <div key={movie.movie.id} className="snap-center w-full h-auto">
        <MovieCard
          ...
          onDelete={canDelete ? () => handleDelete(movie.movie.id) : null}
        />
      </div>
    ))
  )}
